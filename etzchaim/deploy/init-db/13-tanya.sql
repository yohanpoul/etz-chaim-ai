...
-- TimescaleDB : série temporelle (optionnel).
DO $$
BEGIN
    PERFORM create_hypertable('beinoni_interactions', 'created_at',
                              if_not_exists => true, migrate_data => true);
EXCEPTION WHEN undefined_function OR OTHERS THEN
    RAISE NOTICE 'beinoni_interactions : skipping create_hypertable (timescaledb missing).';
END$$;
...
DO $$
BEGIN
    PERFORM create_hypertable('beinoni_events', 'created_at',
                              if_not_exists => true, migrate_data => true);
EXCEPTION WHEN undefined_function OR OTHERS THEN
    RAISE NOTICE 'beinoni_events : skipping create_hypertable (timescaledb missing).';
END$$;
