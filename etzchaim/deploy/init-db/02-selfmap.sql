...
-- TimescaleDB : routing_log comme série temporelle (optionnel).
-- Si timescaledb n'est pas installé, la table reste une table Postgres standard.
DO $$
BEGIN
    PERFORM create_hypertable('selfmap_routing_log', 'created_at',
                              if_not_exists => true, migrate_data => true);
EXCEPTION WHEN undefined_function OR OTHERS THEN
    RAISE NOTICE 'selfmap_routing_log : skipping create_hypertable (timescaledb missing).';
END$$;
